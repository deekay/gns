...
> today, and which ones would require custom ONT tooling?
...
- but it likely stops being a normal "export PSBT and sign in a traditional
  wallet" flow
- and starts becoming a **custom ONT builder / signer / finalizer** flow
...
| Carrier Option | Sparrow / Traditional Desktop Wallet | Hardware-Wallet PSBT Flow | Custom ONT CLI Signer | Hosted Coordinator / Managed Flow | Notes |
| --- | --- | --- | --- | --- | --- |
...
| Off-chain proof artifact with minimal on-chain reference | mixed | mixed | good | good | Smallest footprint, but weakest fit for ONT trust model |
...
> annex-based ONT reveals might be possible with custom work, but they are
...
If ONT ever offered a more managed path, a coordinator could:
...
For ONT right now, I would summarize it like this:
