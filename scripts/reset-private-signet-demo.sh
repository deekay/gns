...
rm -f /var/lib/gns/gns-prod-demo-snapshot.json
rm -f /var/lib/gns/private-signet-resolver-snapshot.json
rm -f /var/lib/gns/private-signet-value-records.json

echo
echo "[wipe database-backed resolver docs]"
if [[ -f /etc/gns/gns.env ]]; then
  GNS_DATABASE_URL=$(awk -F= '/^GNS_DATABASE_URL=/{sub(/^[^=]*=/,""); print; exit}' /etc/gns/gns.env)
  GNS_DATABASE_SCHEMA=$(awk -F= '/^GNS_DATABASE_SCHEMA=/{print $2; exit}' /etc/gns/gns.env)

  if [[ -n "${GNS_DATABASE_URL:-}" ]]; then
    runuser -u gns -- env \
      GNS_DATABASE_URL="${GNS_DATABASE_URL}" \
      GNS_DATABASE_SCHEMA="${GNS_DATABASE_SCHEMA:-public}" \
      bash -lc 'cd /opt/gns/app && node <<'"'"'NODE'"'"'
const { Client } = require("pg");

const schema = process.env.GNS_DATABASE_SCHEMA || "public";
if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(schema)) {
  throw new Error(`invalid schema name: ${schema}`);
}

(async () => {
  const client = new Client({ connectionString: process.env.GNS_DATABASE_URL });
  await client.connect();
  await client.query(`delete from "${schema}"."gns_documents" where kind = any($1) and document_key = $2`, [
...
if [[ "$DELETE_LOCAL" == "1" ]]; then
  ...
  rm -rf "$HOME/Downloads/gns-demo"
  rm -f "$HOME/Downloads/gns-claim-moneyball-signer-notes.txt"
  rm -rf "$ROOT_DIR/.data/private-signet-demo"
  rm -f "$HOME/.sparrow/signet/wallets/gns-demo.mv.db"
  rm -f "$HOME/.sparrow/signet/wallets/gns-demo-2.mv.db"
fi
